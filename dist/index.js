    ...
    RedirectableRequest.prototype._processResponse = function (response) {
      ...
      if (location && this._options.followRedirects !== false && statusCode >= 300 && statusCode < 400) {
        ...
        var previousHostName =
          removeMatchingHeaders(/^host$/i, this._options.headers) || url.parse(this._currentUrl).hostname;
...
var require_compareAsc = __commonJS({
  'node_modules/date-fns/compareAsc/index.js'(exports2, module2) {
    ...
    exports2.default = compareAsc2;
    var _index = _interopRequireDefault(require_toDate());
    var _index2 = _interopRequireDefault(require_requiredArgs());
    function _interopRequireDefault(obj) {
      return obj && obj.__esModule ? obj : { default: obj };
    }
    function compareAsc2(dirtyDateLeft, dirtyDateRight) {
...
var compareAsc = require_compareAsc();
...
var branchCleanupStrategy = core.getInput('branch-cleanup-strategy', requiredArgOptions);
var tagCleanupStrategy = core.getInput('tag-cleanup-strategy', requiredArgOptions);
var shaCleanupStrategy = core.getInput('sha-cleanup-strategy', requiredArgOptions);
var branchThreshold = core.getInput('branch-threshold', requiredArgOptions);
var tagThreshold = core.getInput('tag-threshold', requiredArgOptions);
var shaThreshold = core.getInput('sha-threshold', requiredArgOptions);
var projectBoardNumber = core.getInput('board-number', requiredArgOptions);
var owner = github.context.repo.owner;
var repo = github.context.repo.repo;
var orgAndRepo = `${owner}/${repo}`;
    ...
    response.repository.issues.edges.forEach(rawIssueNode => {
      ...
      if (rawIssue.labels && rawIssue.labels.edges && rawIssue.labels.edges.length > 0) {
        issue.labels = rawIssue.labels.edges
          .filter(l => l.node.name.includes('\u{1F680}currently-in-'))
          .map(l => l.node.name);
...
async function getActiveBranches() {
  try {
    ...
    const activeBranches2 = response.data.map(b => b.name.toLowerCase());
...
There are no inactive ${cardType} cards to remove.`);
  ...
  const itemsToRemove2 = inactiveItems.filter(i => compareAsc(i.updatedAt, maxDate) === -1);
  core.info(`
...
Inactive ${cardType} Cards to Remove based on Max Age of ${maxAgeInDays} Days (created before ${maxDateString}):`);
  if (itemsToRemove2 && itemsToRemove2.length > 0) {
    itemsToRemove2.forEach(b => core.info(`- ${b.updatedAtString} - ${b.title}`));
  } else {
    core.info(`- There are no inactive ${cardType} cards to cleanup`);
  }
  return itemsToRemove2;
}
...
Inactive ${cardType} Cards to Keep based on Max Number of ${maxNumberOfItems} Inactive Items:`);
  ...
  const itemsToRemove2 = inactiveItems.slice(spliceAt);
  core.info(`
